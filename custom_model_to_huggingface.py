...
def load_process_train(path, name, random_state=42, *args, **kwargs):
    ...
    # Load data
    df = pd.read_csv(path)
    df[cmd_args.df_sentence] = df[cmd_args.df_sentence].astype('str')
    
    # X-y split, vectorize X
    X = df.drop([cmd_args.df_tag], axis=1)
...
def model_to_huggingface(path, model, repo_id, model_filename, commit_message):
    """
...
model_to_huggingface(path_file, passAgg, repo_id, model_filename, commit_message)
